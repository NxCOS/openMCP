...
def createDatabase(hostAddr, dbName, dbUser, dbPass):
  ...
  try:
    ...
    if (checkDatabase(dbCursor, dbName)) == False:
      ...
      userInput = input("Would you like to re-create the database (y/n)?: ").lower()
      if userInput in ('y', 'yes'):
        print("OK!...")
        createUserDefined(dbCursor, dbName, 'DROP')
      else:
  ...
  # Check if 'gametitle' table does not exist
    ...
    # Create the table and its parms
      dbSQL = """
                CREATE TABLE IF NOT EXISTS `gametitle` (
                    `gameName` varchar(45) NOT NULL PRIMARY KEY COMMENT 'GameTitle',  
                    `portTCPNumbers` varchar(28) NOT NULL COMMENT 'Server TCP listen ports',  
                    `portUDPNumbers` varchar(28) NOT NULL COMMENT 'Server UDP listen ports')"""
...
def createUserDefined(cursor, databaseName, input):
  ...
  if (input == 'DROP'):
    ...
    dbSQL = "DROP DATABASE `{0}`".format(databaseName)
